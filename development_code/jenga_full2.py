...
class PickAndPlace(object):
    ...
    def _retract(self, orientation_mode): # orientation = "top" or "side"
    ...
    def pick(self, pose, orientation_mode):
        # open the gripper
        self.gripper_open()
        # servo above pose
        print('Approaching')
        self._approach(pose, orientation_mode)
        # servo to pose
        ...
        self._retract(orientation_mode)

    # Place method applying a combination of other methods
    def place(self, pose, orientation_mode):
        # servo above pose
        self._approach(pose, orientation_mode)
        # servo to pose
        self._servo_to_pose(pose)
...
def main():
    ...
    play_pose["1"]   = create_pose(0.6598,           0.07 - 2*gap,  0.270 - off,            quat4) # Pick up brick 6  #0.07 - 2*trim
    play_pose["2"]   = create_pose(0.6598,           0.0,           0.531 - off,            quat5) # Place brick 6 in position 14

    clear_play = {} # Poses for clearing layer 2 in play

    clear_play["1"]  = create_pose(0.5897,           0.3333,        0.357 - off,            quat3)
    clear_play["1b"] = create_pose(0.5898,           0.3333,        0.357 - off,            quat4)
    clear_play["1c"] = create_pose(0.5898,           0.3333,        0.357 - off,            quat5)
    clear_play["1d"] = create_pose(0.5898,           0.3333,        0.531 - off,            quat5)
    ...
    if build_tower == True:
        ...
        for layer in [1, 2, 3, 4]: # 4 vertical layers in the tower
            for brick in [1, 2, 3]: # 3 bricks in each layer
                ...
                print("Picking...")
                left_pnp.pick(pick_pose, "top") # Pick brick up from collection site
                if layer in [3, 4]: left_pnp.move_to(left_pnp.ik_request(prep_pose[str(layer)])) # Intermediate preparation pose
                print("Placing...")

                left_pnp.place(place_pose[brick_id],"top") # Place brick on tower
    ...
    if play_jenga == True:
        # Jenga sequence
        print("\nPicking...")
        left_pnp.move_to(left_pnp.ik_request(clear_play["1"])) # Clearance from tower
        left_pnp.move_to(left_pnp.ik_request(clear_play["1b"])) # Clearance from tower
        left_pnp.pick(play_pose["1"], "side") # Pick up brick 6 from tower

        rospy.sleep(1.0)

        left_pnp.move_to(left_pnp.ik_request(clear_play["1b"])) # Clearance from tower
        left_pnp.move_to(left_pnp.ik_request(clear_play["1c"])) # Clearance from tower

        print("\nPlacing...")
        left_pnp.move_to(left_pnp.ik_request(clear_play["1d"])) # Clearance from tower
        left_pnp.place(play_pose["2"], "top") # Place brick 6 on top of tower in position 14
