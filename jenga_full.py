...
def main():
    ...
    quat4 = tf.transformations.quaternion_from_euler(-1.5708, -1.57080, -3.14159)
    # rotate end-effector 90 degrees about z (from quat4)
    quat5 = tf.transformations.quaternion_from_euler(3.14159, -1.57080, 0)
    ...
    play_pose["1"]  = create_pose(0.6598,          0.07 - 2*gap,    0.27 - off,  quat4) # Pick up brick 6  #0.07 - 2*trim
    ...
    for layer in [1, 2, 3, 4]: # 4 vertical layers in the tower
        for brick in [1, 2, 3]: # 3 bricks in each layer
            ...
            print("Picking...")
            left_pnp.pick(pick_pose, 1) # Pick brick up from collection site
            if layer in [3, 4]: left_pnp.move_to(left_pnp.ik_request(prep_pose[str(layer)])) # Intermediate preparation pose
            print("Placing...")

            left_pnp.place(place_pose[brick_id], 1) # Place brick on tower
    ...
    left_pnp.place(play_pose["2"], 1) # Place brick 6 on top of tower in position 14
